...
function Input({ label, ...props }) {
  return (
    <div>
      ...
      <input {...props} style={{width:'100%',background:'#0a0e1a',border:'1px solid #1e293b',borderRadius:'10px',padding:'11px 14px',fontSize:'14px',color:'#f1f5f9',fontFamily:'inherit',outline:'none',...props.style}}
...
function Select({ label, children, ...props }) {
  return (
    <div>
      {label && <label style={{display:'block',fontSize:'11px',fontWeight:600,marginBottom:'8px',textTransform:'uppercase',letterSpacing:'0.8px',color:'#64748b'}}>{label}</label>}
      <select {...props} style={{width:'100%',background:'#0a0e1a',border:'1px solid #1e293b',borderRadius:'10px',padding:'11px 14px',fontSize:'14px',color:'#f1f5f9',fontFamily:'inherit',outline:'none',cursor:'pointer',...props.style}}
...
export default function Dashboard() {
  ...
  return (
    <div style={{minHeight:'100vh',background:'#080c14',fontFamily:"'Inter',system-ui,sans-serif"}}>
      ...
      {/* ── NAVBAR ── */}
      <nav style={{position:'sticky',top:0,zIndex:50,height:'60px',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 32px',background:'rgba(8,12,20,0.92)',backdropFilter:'blur(24px)',borderBottom:'1px solid #0f1e35'}}>
        <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
          <div style={{width:'34px',height:'34px',borderRadius:'10px',background:'linear-gradient(135deg,#6366f1,#8b5cf6)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'16px',boxShadow:'0 0 20px rgba(99,102,241,0.4)'}}>🎯</div>
          <span style={{fontFamily:'monospace',fontWeight:700,fontSize:'14px',letterSpacing:'3px',color:'#e2e8f0'}}>HIRETRACK</span>
          <span style={{marginLeft:'8px',padding:'2px 10px',borderRadius:'20px',fontSize:'10px',fontWeight:600,background:'rgba(99,102,241,0.15)',color:'#818cf8',border:'1px solid rgba(99,102,241,0.25)',letterSpacing:'1px'}}>BETA</span>
        </div>
        <div style={{display:'flex',alignItems:'center',gap:'12px'}}>
          <div style={{display:'flex',alignItems:'center',gap:'8px',padding:'6px 14px',borderRadius:'30px',background:'#0f1829',border:'1px solid #1e293b',fontSize:'13px',color:'#94a3b8'}}>
            <div style={{width:'26px',height:'26px',borderRadius:'50%',background:'linear-gradient(135deg,#6366f1,#8b5cf6)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'11px',fontWeight:700,color:'white'}}>
              {user?.name?.charAt(0).toUpperCase()}
            </div>
            <span style={{color:'#cbd5e1',fontWeight:500}}>{user?.name}</span>
          </div>
          <button onClick={()=>{logout();navigate('/login')}} style={{padding:'7px 16px',borderRadius:'8px',background:'transparent',border:'1px solid #1e293b',color:'#475569',fontSize:'13px',cursor:'pointer',fontFamily:'inherit',transition:'all 0.2s'}}
      ...
      <div style={{maxWidth:'1160px',margin:'0 auto',padding:'36px 32px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'16px',marginBottom:'28px'}}>
          {STATS_CONFIG.map((s,i)=>(
            <div key={i} style={{background:'#0d1117',border:'1px solid #161f2e',borderRadius:'16px',padding:'22px 24px',position:'relative',overflow:'hidden',cursor:'default',transition:'transform 0.2s,border-color 0.2s'}}
              onMouseEnter={e=>{e.currentTarget.style.transform='translateY(-3px)';e.currentTarget.style.borderColor=s.accent+'44'}}
              onMouseLeave={e=>{e.currentTarget.style.transform='translateY(0)';e.currentTarget.style.borderColor='#161f2e'}}>
              {/* glow */}
              <div style={{position:'absolute',top:0,right:0,width:'80px',height:'80px',background:s.glow,borderRadius:'0 16px 0 80px',pointerEvents:'none'}}/>
              {/* accent bar */}
              <div style={{position:'absolute',top:0,left:0,width:'3px',height:'100%',background:s.accent,borderRadius:'16px 0 0 16px'}}/>
              <div style={{fontSize:'11px',fontWeight:600,textTransform:'uppercase',letterSpacing:'1.2px',color:'#334155',marginBottom:'14px'}}>{s.label}</div>
              <div style={{fontSize:'42px',fontWeight:700,lineHeight:1,color:s.accent,fontFamily:'monospace',marginBottom:'8px'}}>{s.value}</div>
              <div style={{fontSize:'12px',color:'#334155'}}>{s.sub}</div>
            </div>
          ))}
        </div>
                ...
                {loading ? (
                  ...
                  return (
                    <tr key={job.id} style={{borderBottom:'1px solid #0f1829',transition:'background 0.15s',cursor:'default'}}
                      ...
                      {/* status */}
                      <td style={{padding:'16px 20px'}}>
                        <span style={{display:'inline-flex',alignItems:'center',gap:'6px',padding:'5px 12px',borderRadius:'20px',fontSize:'11px',fontWeight:600,letterSpacing:'0.5px',background:s.bg,color:s.text,border:`1px solid ${s.border}`}}>
                      ...
                      <td style={{padding:'16px 20px'}}>
                        <div style={{display:'flex',gap:'8px'}}>
                          ...
                          <button onClick={()=>handleDelete(job.id)} style={{padding:'6px 14px',borderRadius:'8px',background:'rgba(239,68,68,0.06)',border:'1px solid rgba(239,68,68,0.15)',color:'#f87171',fontSize:'12px',fontWeight:500,cursor:'pointer',fontFamily:'inherit',transition:'all 0.15s'}}
          ...
          <div style={{width:'100%',maxWidth:'460px',background:'#0d1117',border:'1px solid #1e293b',borderRadius:'20px',padding:'32px',boxShadow:'0 40px 80px rgba(0,0,0,0.7)',animation:'slideUp 0.25s ease'}}>
            ...
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'28px'}}>
              <div>
                ...
                <p style={{fontSize:'12px',color:'#475569',marginTop:'3px'}}>{editId?'Update the details below':'Track a new job application</p>'}</p>
              </div>
      ...
      {/* ── TOAST ── */}
      {toast && (
        <div style={{position:'fixed',bottom:'28px',right:'28px',zIndex:200,padding:'14px 20px',borderRadius:'12px',fontSize:'14px',fontWeight:500,display:'flex',alignItems:'center',gap:'10px',boxShadow:'0 20px 60px rgba(0,0,0,0.5)',animation:'slideUp 0.3s ease',
